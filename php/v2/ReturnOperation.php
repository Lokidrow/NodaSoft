...
class TsReturnOperation extends ReferencesOperation {
	...
	public function doOperation(): array {
		...
		if (!is_array($data) || empty($data)) {
			throw new Exception('Request response incorrect');
		}
		...
		if (empty($client) || empty($client->type) || empty($client->Seller->id) || $client->type !== Contractor::TYPE_CUSTOMER
		...
		if ($notificationType === self::TYPE_CHANGE && !empty($data['differences']['to'])) {
			...
			if (!empty($client->mobile)) {
				$error = '';
				$res = NotificationManager::send($resellerId, $client->id, NotificationEvents::CHANGE_RETURN_STATUS, (int)$data['differences']['to'],
					$templateData, $error);
				if ($res) {
